...
class EvaluationManager:
    ...
    def __init__(self, config: EvaluationConfig):
        ...
        logger.info(
            ...
            extra={
                ...
                "langsmith_available": self.langsmith_available,
    ...
    def _check_dependencies(self):
        ...
        if not self.langsmith_available:
            logger.warning(
                ...
                extra={
                    "missing_packages": ["langsmith", "openevals"],
                    "install_command": "pip install langsmith openevals",
                    "fallback": "Will use built-in heuristic evaluators"
                }
            )
        else:
            logger.debug("LangSmith dependencies available")
    ...
    def _setup_evaluators(self):
        ...
        # Always set up built-in evaluators as fallbacks
        default_evaluators = {
        ...
        # If no evaluators are configured, use default built-ins
        if not self.config.evaluators:
            logger.info(
                "No evaluators configured, using default built-in evaluators",
                extra={"default_evaluators": list(default_evaluators.keys())}
            )
            self.evaluators.update(default_evaluators)
            return
        
        # Process configured evaluators
        for evaluator_config in self.config.evaluators:
            if evaluator_config.enabled:
                ...
                if evaluator is not None:
                    ...
                    logger.debug(f"Added evaluator: {evaluator_config.name} ({evaluator_config.type})")
                else:
                    # Fall back to built-in if available
                    if evaluator_config.name in default_evaluators:
                        self.evaluators[evaluator_config.name] = default_evaluators[evaluator_config.name]
                        logger.warning(
                            f"Failed to create configured evaluator '{evaluator_config.name}', using built-in fallback"
                        )
        
        # Ensure we have at least some evaluators
        if not self.evaluators:
            logger.warning("No evaluators could be created, falling back to built-in evaluators")
            self.evaluators.update(default_evaluators)
        ...
        **kwargs
        ...
        for evaluator_name in evaluator_names:
            ...
            try:
                ...
                if evaluator_name == "response_time" and start_time and end_time:
                    evaluator_kwargs["start_time"] = start_time
                    evaluator_kwargs["end_time"] = end_time
                elif evaluator_name == "response_time" and "response_time" in output_data:
                    # Use response time from output data if available
                    pass
                
                # Add any additional kwargs passed to the method
                evaluator_kwargs.update(kwargs)
                
                result = evaluator.evaluate(**evaluator_kwargs)
                ...
                logger.debug(
                    ...
                    extra={
                        ...
                        if result.get("reasoning", "") else ""
                    }
                )
                
            except Exception as e:
                logger.error(
                    f"Evaluator '{evaluator_name}' failed",
                    extra={
                        "error": str(e),
                        "error_type": type(e).__name__,
                        "evaluator_type": getattr(evaluator, '__class__', {}).get('__name__', 'Unknown')
                    },
                    exc_info=True
                )
                results[evaluator_name] = {
                    ...
                    "metadata": {"evaluator": evaluator_name, "error_type": type(e).__name__}
                }
