    ...
    def evaluate(
        ...
        **kwargs
        ...
        needs_search = any(indicator in query for indicator in 
                          ["search", "find", "lookup", "current", "recent", "latest"])
        needs_calculation = any(indicator in query for indicator in 
                               ["calculate", "compute", "math", "number", "count"])
        needs_file_ops = any(indicator in query for indicator in 
                            ["file", "read", "write", "save", "load"])
        
        tools_used = len(tool_results) > 0
        
        if (needs_search or needs_calculation or needs_file_ops) and tools_used:
            score += 0.3
            reasoning_parts.append("Used tools appropriately for the task")
        elif (needs_search or needs_calculation or needs_file_ops) and not tools_used:
            score -= 0.3
            reasoning_parts.append("Should have used tools but didn't")
        elif not (needs_search or needs_calculation or needs_file_ops) and not tools_used:
            score += 0.1
        ...
        if iteration_count > 0:
            if iteration_count <= 3:
                score += 0.2
                reasoning_parts.append("Efficient tool usage with few iterations")
        ...
        if tool_errors > 0:
            score -= 0.2 * (tool_errors / len(tool_results))
            reasoning_parts.append(f"Had {tool_errors} tool errors")
